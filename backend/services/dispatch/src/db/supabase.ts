export async function supabaseQuery<T>(table: string, query = "", init: RequestInit = {}): Promise<T> {
  ...
  const response = await fetch(`${url}/rest/v1/${table}${query ? `?${query}` : ""}`, { ...init, headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": "application/json", ...(init.body ? { Prefer: "return=representation" } : {}), ...init.headers } });
